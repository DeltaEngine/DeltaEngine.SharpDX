...
namespace DeltaEngine.Rendering3D.Particles
{
	...
	public struct Particle3D : Lerp<Particle3D>, Particle
	{
		...
		public static VertexPosition3DColorUV[] GetVertices(Size size, Color color)
		{
			...
			return new[]
			{
				new VertexPosition3DColorUV(new Vector3D(-halfWidth, 0, halfHeight), color, Vector2D.Zero),
				new VertexPosition3DColorUV(new Vector3D(halfWidth, 0, halfHeight), color, Vector2D.UnitX),
				new VertexPosition3DColorUV(new Vector3D(halfWidth, 0, -halfHeight), color, Vector2D.One),
				new VertexPosition3DColorUV(new Vector3D(-halfWidth, 0, -halfHeight), color, Vector2D.UnitY)
			};
		}
			...
			Vector3D startVelocityVariance)
		{
			...
			CurrentMovement = new Vector3D(velocityMin.X.Lerp(velocityMax.X, Randomizer.Current.Get()),
