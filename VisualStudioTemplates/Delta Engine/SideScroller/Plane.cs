...
namespace $safeprojectname$
{
	...
	public abstract class Plane : Sprite
	{
		...
		protected ParticleSystem machingeGunAndLauncher;
		...
		public void FireMgShotIfAllowed()
		{
			...
			{
				machingeGunAndLauncher.AttachedEmitters[0].Spawn();
		...
		public void FireMissileIfAllowed()
		{
			...
			{
				machingeGunAndLauncher.AttachedEmitters[1].Spawn();
		...
		protected class TimeAndPositionUpdate : UpdateBehavior
		{
			public override void Update(IEnumerable<Entity> entities)
			{
				foreach (var entity in entities)
				{
					...
					plane.machingeGunAndLauncher.Position = new Vector3D(plane.Center);
					plane.machingeGunAndLauncher.Rotation = Quaternion.FromAxisAngle(Vector3D.UnitZ,
						plane.Rotation);
				}
			}
		}
		...
		public override bool IsActive
		{
			...
			{
				if (!value)
					foreach (var emitter in machingeGunAndLauncher.AttachedEmitters)
