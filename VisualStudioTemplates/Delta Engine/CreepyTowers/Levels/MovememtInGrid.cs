...
namespace $safeprojectname$.Levels
{
	public class MovementInGrid : UpdateBehavior
	{
		...
		private void MoveCreepAlongZAxis(Creep creep, Vector3D finalStop, float velocity)
		{
			var dir = finalStop - creep.Position;
			var distToMovePerFrame = Vector3D.Normalize(dir).Z * velocity * Time.Delta;
			creep.Position = new Vector3D(creep.Position.X, creep.Position.Y, creep.Position.Z + 
				distToMovePerFrame);
			creep.UpdateHealthBarPositionAndImage();
			CheckIfCreepReachedFinalStop(creep, finalStop);
		}
