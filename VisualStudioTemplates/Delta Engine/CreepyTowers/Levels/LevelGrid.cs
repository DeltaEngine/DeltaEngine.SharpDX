...
namespace $safeprojectname$.Levels
{
	public class LevelGrid : Entity
	{
		...
		private void DrawGrid()
		{
			...
			{
				xLineStart = new Vector3D(-HalfGridSize * GridScale + XOffset, axisXy.Y * GridScale - 
					YOffset, 0.0f);
		...
		private void AssignGridPositions()
		{
			...
			for (int x = 0; x < GridSize; x++)
			{
				...
				y = 0;
				topLeft = new Vector3D(PropertyMatrix [x, y].TopLeft.X - GridScale, PropertyMatrix [x, 
		...
		private void CreateDrawAreaCoordinates(int x, int y)
		{
			...
			PropertyMatrix [x, y].BottomRight = new Vector3D(topLeft.X - GridScale, topLeft.Y - 
				GridScale, topLeft.Z);
			PropertyMatrix [x, y].MidPoint = new Vector3D(topLeft.X - GridScale * 0.5f, topLeft.Y - 
