...
namespace $safeprojectname$
{
	public class LevelGrid : Entity
	{
		...
		public static float HalfGridSize
		{
			get;
			private set;
		}
		...
		public GridProperties[,] PropertyMatrix
		{
			get;
			private set;
		}
		...
		private void DrawGrid()
		{
			...
			{
				xLineStart = new Vector(-HalfGridSize * GridScale + XOffset, axisXy.Y * GridScale - 
					YOffset, 0.0f);
				xLineEnd = new Vector(HalfGridSize * GridScale + XOffset, axisXy.Y * GridScale - YOffset, 
					0.0f);
				new Line3D(xLineStart, xLineEnd, Color.White);
				yLineStart = new Vector(axisXy.X * GridScale + XOffset, -HalfGridSize * GridScale - 
					YOffset, 0.0f);
		...
		private void AssignGridPositions()
		{
			...
			for (int x = 0; x < GridSize; x++)
			{
				...
				y = 0;
				topLeft = new Vector(PropertyMatrix [x, y].TopLeft.X - GridScale, PropertyMatrix [x, 
					y].TopLeft.Y, PropertyMatrix [x, y].TopLeft.Z);
			}
		}
		...
		private void CreateDrawAreaCoordinates(int x, int y)
		{
			...
			PropertyMatrix [x, y].BottomRight = new Vector(topLeft.X - GridScale, topLeft.Y - 
				GridScale, topLeft.Z);
			PropertyMatrix [x, y].MidPoint = new Vector(topLeft.X - GridScale * 0.5f, topLeft.Y - 
				GridScale * 0.5f, topLeft.Z);
		}
			...
			ChangeableList<Tuple<int, int>> interactablePointsList)
		{
			...
			foreach (var tuple in interactablePointsList)
			{
				...
				if (clickedPoint.X >= bottomRight.X && clickedPoint.X < topRight.X && clickedPoint.Y >= 
					topRight.Y && clickedPoint.Y < topLeftPoint.Y)
				{
