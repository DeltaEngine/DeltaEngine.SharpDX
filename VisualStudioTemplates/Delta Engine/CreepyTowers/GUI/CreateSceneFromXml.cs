...
namespace $safeprojectname$.GUI
{
	public class CreateSceneFromXml : Scene
	{
		...
		public List<InteractiveButton> InteractiveButtonList
		{
			get;
			private set;
		}
		...
		private void CreateUiElement(UIObject uiObject, Rectangle drawArea)
		{
			...
			if (Names.ComicStripImages.Contains(uiObject.Name))
			{
				...
				if (uiObject.Name.Equals(Names.ComicStripBubble))
					AddTextToScene();
				Add(comicStripImage);
			} else if (uiObject.Name.Equals(Names.IconMouseLeft) || 
				uiObject.Name.Equals(Names.IconMouseRight))
			{
		...
		{
			foreach (InteractiveButton control in Controls.Where(control => 
				control.ContainsTag(Names.ButtonMessageBack)))
			{
