...
namespace $safeprojectname$
{
	public class WaveXmlParser
	{
		...
		public Queue<Wave> WaveObjectsList
		{
			get;
			private set;
		}

		public List<Tuple<int, int>> SpawnPointsList
		{
			get;
			private set;
		}

		public List<Tuple<int, int>> ExitPointsList
		{
			get;
			private set;
		}
		...
		public void ParseXml(string xmlName)
		{
			...
			GridSize = new Size(int.Parse(size [0]), int.Parse(size [1].Trim()));
			var gold = xmlContent.Data.GetAttributeValue("StartingGold");
			Gold = int.Parse(gold, CultureInfo.InvariantCulture);
