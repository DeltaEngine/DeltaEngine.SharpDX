...
namespace $safeprojectname$.Towers
{
	public sealed class Tower : DrawableEntity, Actor
	{
		...
		private void FireParticle(Creep creep)
		{
			...
			particleEmitterData.StartRotation = new RangeGraph<ValueRange>(new ValueRange(-angle, 
				-angle), new ValueRange(-angle, -angle));
		...
		private static ParticleEmitterData CreateParticleEmitterData()
		{
			...
			var data = new ParticleEmitterData {
				...
				StartVelocity = new RangeGraph<Vector3D>(new Vector3D(-0.5f, -0.5f, 0.1f), new 
					Vector3D(0.5f, 0.5f, 0.1f)),
