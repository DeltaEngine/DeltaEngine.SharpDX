...
namespace $safeprojectname$
{
	public class Game : Entity
	{
		...
		private void AddCommands(LookAtCamera camera)
		{
			new Command(() => 
			{
				MoveCameraToLeft(camera);
			}).Add(new KeyTrigger(Key.A, State.Pressed));
			new Command(() => 
			{
				MoveCameraToRight(camera);
			}).Add(new KeyTrigger(Key.D, State.Pressed));
			new Command(() => 
			{
				camera.Zoom(0.5f);
			}).Add(new KeyTrigger(Key.W, State.Pressed));
			new Command(() => 
			{
				camera.Zoom(-0.5f);
			}).Add(new KeyTrigger(Key.S, State.Pressed));
