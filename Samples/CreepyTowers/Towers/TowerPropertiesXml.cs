		...
		private static TowerData ParseTowerData(XmlData tower)
		{
			return
				new TowerData((TowerType)Enum.Parse(typeof(TowerType), tower.GetAttributeValue("Type")),
					...
					int.Parse(tower.GetAttributeValue("Cost"), CultureInfo.InvariantCulture));
		}

		private readonly TowerData[] towerData =
			new TowerData[Enum.GetNames(typeof(TowerType)).Length];
