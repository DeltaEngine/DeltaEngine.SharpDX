...
{
	/// <summary>
	/// GameLevelRoom class. It has all important functionality for the level itself 
	/// </summary>
	public class GameLevelRoom : Level
	{
		public GameLevelRoom(Size size)
			: base(size)
		{
		...
		protected GameLevelRoom(string contentName)
			: base(contentName) {}
		...
		public Creep SpawnCreep(CreepType type)
		{
			...
			var creep = new Creep(type, SpawnPoints[0], 0.0f) { Target = GoalPoints[0] };
			creep.Path = list;
			creeps.Add(creep);
			return creep;
		}
