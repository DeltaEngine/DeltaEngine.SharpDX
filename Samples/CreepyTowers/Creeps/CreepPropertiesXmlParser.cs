...
namespace CreepyTowers.Creeps
{
	...
	public class CreepPropertiesXmlParser
	{
		...
		public Dictionary<Creep.CreepType, CreepProperties> CreepPropertiesData { get; private set; }
		...
		private static CreepProperties CreateCreepProperties(XmlData creep)
		{
			return new CreepProperties
			{
				...
				CreepType =
					(Creep.CreepType)Enum.Parse(typeof(Creep.CreepType), creep.GetAttributeValue("Type")),
				MaxHp = float.Parse(creep.GetAttributeValue("MaxHp"), CultureInfo.InvariantCulture),
				Resistance = float.Parse(creep.GetAttributeValue("Resistance"), CultureInfo.InvariantCulture),
