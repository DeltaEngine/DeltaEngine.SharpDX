	...
	public class PathFindingGrid
	{
		...
		private void CreateGridLines()
		{
			for (int i = -halfGridRowSize; i <= halfGridRowSize; i++)
				...
				{
					new Line3D(new Vector(-halfGridColumnSize * gridScale, i * gridScale, 0.0f),
						new Vector(halfGridColumnSize * gridScale, i * gridScale, 0.0f), Color.White);
					new Line3D(new Vector(j * gridScale, -halfGridRowSize * gridScale, 0.0f),
						new Vector(j * gridScale, halfGridRowSize * gridScale, 0.0f), Color.White);
				}
		}
		...
		private void PaintGridCenter()
		{
			for (int i = 0; i < gridColumnSize; i++)
				for (int j = 0; j < gridRowSize; j++)
				{
					...
					var x = Vector.UnitX.X * gridScale * -halfGridColumnSize + gridScale / 2.0f + i * gridScale;
