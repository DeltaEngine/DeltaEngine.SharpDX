...
namespace DeltaEngine.Rendering.Particles
{
	internal class Particle3DRenderer : DrawBehavior
	{
		...
		{
			var interpolatedParticleArray = emitter.GetInterpolatedArray<Particle3D>();
		...
		{
			Vector particlePositionInCameraSpace = device.CameraViewMatrix * particle.Position;
		...
		{
			Vector look = device.CameraInvertedViewMatrix.Translation - particle.Position;
			look.Normalize();
