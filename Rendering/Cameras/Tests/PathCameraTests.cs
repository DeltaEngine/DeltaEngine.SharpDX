...
namespace DeltaEngine.Rendering.Cameras.Tests
{
	public class PathCameraTests : TestWithMocksOrVisually
	{
		...
		private static Matrix[] GetLinearViewMatrixTrack(int length)
		{
			...
			{
				var position = new Vector(0, -(length / 2) + i, 5);
				var target = new Vector(0, -(length / 2) + i + 1, 5);
				track.Add(Matrix.CreateLookAt(position, target, Vector.UnitZ));
			}
		...
		{
			return
				points.Select(point => Matrix.CreateLookAt(point, Vector.Zero, Vector.UnitZ)).ToList();
		}

		private static List<Vector> GetCirclePoints(float radius, float distanceToGround)
		{
		...
		private static Vector CalculateCirclePoint(float radius, float distanceToGround,
			int pointCount, float theta)
		{
			...
			return new Vector(x, y, distanceToGround);
		}
		...
		private static void DrawPoints(IList<Vector> circlePoints)
		{
			for (int i = 0; i < circlePoints.Count; i++)
			{
				var start = circlePoints[i];
				Vector end = i + 1 < circlePoints.Count ? circlePoints[i + 1] : circlePoints[0];
