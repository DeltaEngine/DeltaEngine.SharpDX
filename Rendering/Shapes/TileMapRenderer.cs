	...
	public class TileMapRenderer
	{
		...
		private void CreateForegroundGrid()
		{
			for (int x = 0; x < map.Size.Height + 1; x++)
				lines.Add(new Line2D(map.CalculateGridScreenPosition(new Point(x, 0)),
					map.CalculateGridScreenPosition(new Point(x, (int)map.Size.Height)), Color.Gray)
				{
					RenderLayer = 10
				});
			for (int y = 0; y < map.Size.Width + 1; y++)
				lines.Add(new Line2D(map.CalculateGridScreenPosition(new Point(0, y)),
					map.CalculateGridScreenPosition(new Point((int)map.Size.Width, y)), Color.Gray)
				{
					RenderLayer = 10
				});
		}
